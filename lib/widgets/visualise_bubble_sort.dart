...
class VisualizeBubbleSort extends StatefulWidget {
  const VisualizeBubbleSort({super.key});

  @override
  State<VisualizeBubbleSort> createState() => _VisualizeBubbleSortState();
}

class _VisualizeBubbleSortState extends State<VisualizeBubbleSort> {
    ...
    for (int i = 0; i < numbers.length - 1; i++) {
      for (int j = 0; j < numbers.length - i - 1; j++) {
        ...
        if (numbers[j] > numbers[j + 1]) {
          ...
          setState(() {}); // Show the updated swap
          await Future.delayed(Duration(milliseconds: (400 ~/ speed)));
        }
      }
      setState(() => sortedUpto = numbers.length - i - 1);
    }
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              Center(child: _buildGradientTitle('Bubble Sort Visualizer')),
